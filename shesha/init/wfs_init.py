...
def wfs_init(context: carmaWrap_context, telescope: Telescope, p_wfss: list,
    ...
    for i in range(nsensors):
        ...
        if p_wfs.type == scons.WFSType.PYRHR or p_wfs.type == scons.WFSType.PYRLR:
            ...
            print(type(p_wfs._phasemap), type(p_wfs._hrmap), type(p_wfs._binmap), type(p_wfs._halfxy),
                            type(fluxPerSub), type(p_wfs._validsubsx), type(p_wfs._validsubsy),
                            type(p_wfs._validpuppixx), type(p_wfs._validpuppixy), type(p_wfs._ftkernel))
